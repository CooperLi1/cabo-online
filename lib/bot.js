  ...
  maybeTakeTurn(st) {
    ...
    if (st.stage === 'draw') {
      this.later(delay, () => {
        ...
        // call cabo when we believe we're the lowest at the table. every bot
        // sizes up its opponents (fewer cards → lower likely total), so no
        // one calls into a player sitting on one card
        if (!cur.caboPid) {
          if (this.cfg.master && this.isKamikazeHand(cur)) return this.room.callCabo(this.p);
          const unknowns = this.unknownOwn(cur).length;
          const confident = this.cfg.master ? unknowns <= 1 : unknowns === 0;
          if (confident) {
            const believed = this.ownTotal(cur);
            const minOpp = Math.min(...this.oppEstimates(cur), Infinity);
            const margin = this.cfg.master ? 2 : 1.5;
            if (
              (believed <= this.cfg.caboAt && believed + margin < minOpp) ||
              (this.cfg.master && believed <= 3 && believed + 0.5 < minOpp)
            ) {
