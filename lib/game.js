...
const PEEK_MS = 25000;
const TURN_MS = 45000; // default; hosts pick 10–120s per room
const GIVE_MS = 15000;
const SNAP_GRACE_MS = 450;
...
function powerOf(c) {
  ...
  if (c.r === 'Q') return 'peek-swap'; // peek any card, then optionally swap it with yours
  return null; // kings have no power — black kings are just poison
}
...
class GameRoom {
  ...
  fx(type, data = {}) {
    ...
    if (this.fxLog.length > 14) this.fxLog.shift();
  }
  ...
  setTurnMs(byPlayer, ms) {
    ...
    this.turnMs = Math.round(Math.min(Math.max(ms, 10000), 120000) / 1000) * 1000;
    this.broadcast();
  }
  ...
  endRound(forcedWinnerPid = null) {
    ...
    let winners = [];
    const special = rows.find((r) => r.emptied) || rows.find((r) => r.kamikaze);
    if (special) {
      winners = [special];
    } else {
